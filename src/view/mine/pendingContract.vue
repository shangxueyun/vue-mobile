<template>
    <div style="height: 100%;">
        <div style="position: fixed;top: 0;left: 0;width: 100%;height: 4rem;z-index: 1;">
            <x-header @on-click-back="backReturn('/mine')" style="background-color:#06123C;padding: .6rem 0;" :left-options="{backText: '',preventGoBack:true}">待签订合同</x-header>
        </div>
        ...
        <div v-for="(item,key) in loanList" :key="key" >
            <div style="padding:.8rem 1rem 0.8rem;background-color:#fff;text-align: left;" @click="backReturn" :data="JSON.stringify(item)">
                <cell class="hr_list1" style="font-size:1rem;padding:0;" :title="item.loanAmt" value="待签订合同"></cell>
                <cell class="hr_list1 color_1" style="font-size:1rem;padding:0;color:#999" title="申请借款" :value="'申请日'+item.loanApplyDate"></cell>
            </div>
    ...
    methods: {
        backReturn(str){
            if(str=="/mine"){
                ...
                window.sessionStorage.loanNo = JSON.parse(str.currentTarget.attributes.data.value)["loanNo"];
                window.sessionStorage.fundSideId = JSON.parse(str.currentTarget.attributes.data.value)["fundSideId"];
                this.$router.push({
...
.hr_list1 .weui-cell__ft{
    color: #333;
}
.color_1 .weui-cell__ft{
    color: #999;
}
/* .item_div:last-child{
